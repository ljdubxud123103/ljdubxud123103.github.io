...
页面从移动端开始组织。图库容器常规最大宽度为 980px，1100px 以上扩展至 1320px 并使用三列瀑布流；工具页最大宽度为 1080px。移动端水平 gutter 为 16px，680px 起工具页扩为 28px。常用密度以 6–8px 小间距组织同组控件，以 12–16px 形成控件内距，以 24–34px 分开任务段落。
...
识色工作台在窄屏纵向堆叠，在 680px 起按约 1.35:0.65 分为参考图与分析面板。匹配结果和项目截图从两列开始，680px 为三列、900px 为四列；图库在手机和平板保持双列瀑布流，1100px 以上转为三列，让超宽桌面能同时看到更多电影画面。
